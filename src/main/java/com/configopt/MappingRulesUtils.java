...
public class MappingRulesUtils {
    ...
    // public static boolean validateInsertion(APIcast apicast, final PathNode node,
    // final MappingRule mappingRule) {
    // boolean insert = true;
    ...
    // issues.add(new CollisionIssue(new
    // ArrayList<MappingRule>(Arrays.asList(mappingRule, mr)),
    ...
    // mr.setPath(new Path(mr.getPath().substring(0, mr.getPath().length() - 1)));
    // // remove $
    // insert = false; // stop inserting mappingRule (longer)
    // }
    // insert = true;
    // } else {
    // insert = UserInputManager.requestMappingKeep(mappingRule, mrEndingInThisNode,
    // true); //collision
    // }
    // if (insert && !checkOptimization(mr, mappingRule) &&
    // !UserInputManager.requestMappingKeep(mr, mrEndingInThisNode, false)) //remove
    // existing?
    ...
    // return !m1.matches(m2) && (longer.getPath().startsWith(shorter.getPath()) &&
    // (shorter.getPath().endsWith("$")
    // || (shorter.getPath().length() == longer.getPath().length() &&
    // longer.getPath().endsWith("$"))));
    // }
    ...
    public static void validateMappingRule(APIcast apicast, MappingRuleSM mappingRule,
        ...
        for (int index = i; index < allRulesToVerify.size(); index++) {
            ...
            if (Utils.mode == Mode.SCAN) {
                if (mappingRule.matches(currentRule))
